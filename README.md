...
export default ButtonNumber; // Don’t forget to use export default!
```
...
```js
import React, { Component } from 'react';
import ButtonNumber from './ButtonNumber'; // Import a component from another file

class DangerButton extends Component {
  render() {
    return <ButtonNumber color="red" />;
  }
}
...
We suggest that you stick to using default imports and exports when a module only exports a single thing (for example, a component). That’s what you get when you use `export default ButtonNumber` and `import ButtonNumber from './ButtonNumber'`.
...
### `ButtonNumberNumber.js`

```js
import React, { Component } from 'react';
import './ButtonNumber.css'; // Tell Webpack that ButtonNumber.jsber.js uses these styles

class ButtonNumber extends Component {
  render() {
    // You can use them as regular CSS styles
    return <div className="ButtonNumber" />;
  }
}
```
...
Generally, we recommend that you don’t reuse the same CSS classes across different components. For example, instead of using a `.ButtonNumber` CSS class in `<AcceptButton>` and `<RejectButton>` components, we recommend creating a `<ButtonNumber>` component with its own `.ButtonNumber` styles, that both `<AcceptButton>` and `<RejectButton>` can render (but [not inherit](https://facebook.github.io/react/docs/composition-vs-inheritance.html)).
...
```js
import { Navbar, Jumbotron, ButtonNumber } from 'react-bootstrap';
```
...
Unlike the previous smoke test using `ReactDOM.render()`, this test only renders `<App>` and doesn’t go deeper. For example, even if `<App>` itself renders a `<ButtonNumber>` that throws, this test will pass. Shallow rendering is great for isolated unit tests, but you may still want to create some full rendering tests to ensure the components integrate correctly. Enzyme supports [full rendering with `mount()`](http://airbnb.io/enzyme/docs/api/mount.html), and you can also use it for testing state changes and component lifecycle.
